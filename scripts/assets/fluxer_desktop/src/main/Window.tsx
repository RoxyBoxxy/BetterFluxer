...
function isBenignLoadAbort(error: unknown): boolean {
...
function setupDisplayMediaHandler(session: Electron.Session, webContents: Electron.WebContents): void {
	session.setDisplayMediaRequestHandler((request, callback) => {
		...
		if (process.platform !== 'linux') {
			void fetchDesktopSources(['screen', 'window'])
				.then((sources) => {
					...
					log.debug('[DisplayMedia] Prefetched desktop sources for request:', requestId);
				})
				.catch((error) => {
					log.warn('[DisplayMedia] Failed to prefetch desktop sources for request:', {requestId, error});
				});
		}
			...
			try {
				...
				if (!sources || sources.length === 0) {
					...
					if (hasFreshGlobalCache) {
						...
						log.debug('[selectDisplayMediaSource] Cache miss, resolving sources from desktopCapturer:', requestId);
						sources = await fetchDesktopSources(['screen', 'window']);
					}
				} else {
					log.debug('[selectDisplayMediaSource] Cache hit for request:', requestId);
				}

				let selectedSource = resolveSelectedDesktopSource(sources, sourceId);
				if (!selectedSource && process.platform === 'linux') {
					log.warn('[selectDisplayMediaSource] Linux source cache mismatch; refreshing sources once', {
						requestId,
						sourceId,
					});
					sources = await fetchDesktopSources(['screen', 'window']);
					selectedSource = resolveSelectedDesktopSource(sources, sourceId);
				}
				if (!selectedSource) {
...
export function createWindow(): BrowserWindow {
	...
	if (isLinux) {
		mainWindow.webContents.once('dom-ready', showWindowOnce);
	}

	const showFallbackDelayMs = isLinux ? 1200 : 5000;
	setTimeout(() => {
		if (!windowShown) {
			log.warn(`ready-to-show did not fire within ${showFallbackDelayMs}ms, forcing window to show`);
			showWindowOnce();
		}
	}, showFallbackDelayMs);
	...
	mainWindow.loadURL(appUrl).catch((error) => {
		if (isBenignLoadAbort(error)) {
			logger.info('Initial app URL load was aborted by a follow-up navigation');
			return;
		}
